import React from "react";
import image from "../assets/image.jpg";

function About() {
  return (
    <div>
      <div className="intro-container">
        <div className="">
          {/* <p className="hello">Hello</p> */}
          <span className="handla">I'm Handla</span>
          <p className="hello">I'm a software engineer with years of experience</p>
        </div>
        <img src={image} alt="" className="image-con"></img>
        {/* Lorem ipsum dolor sit amet consectetur adipisicing elit. Corrupti
        doloribus earum facere architecto, sit neque enim sint nisi nemo quidem
        nam sunt eius dolorem vero animi! Praesentium beatae alias explicabo.
        Lorem ipsum, dolor sit amet consectetur adipisicing elit. Sequi, quis
        animi id hic temporibus optio quia mollitia expedita aliquam sit
        possimus! Perspiciatis dignissimos eius, quisquam incidunt dicta rem
        aspernatur? A. Lorem ipsum dolor sit amet consectetur adipisicing elit.
        Sunt ab porro cumque quas inventore maiores blanditiis sint beatae
        maxime aspernatur ipsum ad dolorum facilis rem iure ullam, doloribus
        eveniet mollitia! Lorem ipsum dolor sit amet consectetur adipisicing
        elit. Nesciunt unde temporibus magnam consequatur pariatur harum
        provident quae quis animi veritatis commodi maxime numquam voluptates
        sapiente tempora nihil, totam ipsam corporis? Lorem ipsum dolor, sit
        amet consectetur adipisicing elit. Totam obcaecati ducimus deleniti
        accusamus fugit laboriosam pariatur cupiditate ut, porro corrupti,
        minima quod qui omnis velit. Impedit molestiae placeat suscipit
        consequuntur. Lorem ipsum dolor, sit amet consectetur adipisicing elit.
        Ex ab sit minima tempore perferendis voluptatem quisquam consectetur
        recusandae fuga suscipit soluta voluptate repudiandae magnam dolores
        vero excepturi, a tempora impedit! Lorem ipsum dolor sit amet
        consectetur adipisicing elit. Vel fugit vitae dolore provident eum
        voluptas id quod sequi. Quasi dolor mollitia minima nemo nulla illo!
        Modi provident laborum iste laboriosam.
        <div className="handla">I'm Handla, where from you?</div>
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Esse voluptatem
        adipisci ullam tempore dolor in inventore ratione earum! Eius veniam ea
        voluptates id doloribus debitis est aut atque delectus suscipit. Lorem,
        ipsum dolor sit amet consectetur adipisicing elit. Ab eveniet
        perspiciatis odit a animi architecto mollitia commodi molestias, eaque
        quos, error quas dignissimos eligendi nostrum officia nesciunt unde
        tenetur! Cumque.Perspiciatis */}
      </div>{" "}
    </div>
  );
}

export default About;
